...
import {
    LayoutDashboard, Users, UserCog, CalendarClock,
    BellRing, BrainCircuit, FileBarChart, Search,
    Plus, LogOut, ChevronRight, Database, Menu, X
} from 'lucide-react';
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
    ...
    const navItems = [
        ...
        { name: 'Reservations', href: '/admin/appointments', icon: <CalendarClock size={18} /> },
        { name: 'Patients', href: '/admin/patients', icon: <Users size={18} /> },
        { name: 'Doctors', href: '/admin/doctors', icon: <UserCog size={18} /> },
        { name: 'Schedules', href: '/admin/schedules', icon: <CalendarClock size={18} /> },
        { name: 'AI Knowledge', href: '/admin/knowledge', icon: <BrainCircuit size={18} /> },
        { name: 'Reports', href: '/admin/reports', icon: <FileBarChart size={18} /> },
    ];
                        ...
                        {navItems.map((item) => {
                            ...
                            return (
                                <Link key={item.href} href={item.href} className="block relative group">
                                    <div className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all ${isActive ? "bg-blue-50 text-blue-600 font-semibold" : "text-slate-500 hover:bg-slate-50 hover:text-slate-800"
                                        }`}>
                                        {/* Blue indicator line like Zendenta */}
                                        {isActive && (
                                            <motion.div layoutId="activeNav" className="absolute left-0 w-1 h-5 bg-blue-600 rounded-r-full" />
                                        )}
                                        <span className={`${isActive ? 'text-blue-600' : 'text-slate-300 group-hover:text-slate-500'}`}>
                                            {item.icon}
                                        </span>
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <div className="relative ml-4 hidden md:block group">
                            ...
                            <input
                                ...
                                className="pl-9 pr-4 py-1.5 bg-slate-50 border border-slate-200 rounded-full text-xs w-64 focus:outline-none focus:ring-2 focus:ring-blue-100 focus:bg-white transition-all"
                            />
                        </div>
                    </div>

                    <div className="flex items-center gap-3">
                        {/* Sync AI Button - Integrated */}
                        <button
                        ...
                        <div className="flex items-center gap-3 pl-2">
                            <div className="text-right hidden sm:block">
                                <p className="text-[11px] font-bold text-slate-800 leading-none">Admin Klinik</p>
                                <p className="text-[9px] text-slate-400 font-medium mt-1 uppercase tracking-tighter">Super Admin</p>
                            </div>
                            <img
                                src="https://api.dicebear.com/7.x/avataaars/svg?seed=Admin"
                                className="w-8 h-8 rounded-lg bg-blue-50 border border-slate-200"
                                alt="avatar"
                            />
                        </div>
                    </div>
